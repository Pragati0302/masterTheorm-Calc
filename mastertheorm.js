var app = new (function () {
  ...
  this.calculate = function () {
    ...
    var text = "Error: ";
    if (isNaN(a)) text += "Invalid value of \\(a\\)";
    else if (isNaN(b)) text += "Invalid value of \\(b\\)";
    else if (isNaN(k)) text += "Invalid value of \\(k\\)";
    else if (isNaN(i)) text += "Invalid value of \\(i\\)";
    else if (a < 0) text += "\\(a\\) should be non-negative";
    else if (b <= 1) text += "\\(b\\) should be greater than 1";
    else if (k < 0) text += "\\(k\\) should be at least 0";
    else if (i < 0) text += "\\(i\\) should be at least 0";
    else
      text =
        "\\(T(n) \\: = \\: " +
        (a != 1 ? a : "") +
    ...
    var ans = "\\(T \\: \u2208 \\: \u0398(";
    if (resequal(p, k)) ans += formatPolyLog(k, i + 1);
    else if (p < k) ans += formatPolyLog(k, i);
    else if (p > k) {
      if (resequal(Math.round(p), p)) ans += formatPolyLog(Math.round(p), 0);
      else
        ans +=
    ...
    constructor(id) {
      ...
      function update() {
        ...
        MathJax.Hub.Queue(function () {
          if (preSpanval.parentNode !== null) container.removeChild(preSpanval);
          newSpanval.style.removeProperty("display");
          if (newText === text || newText === null) newText = null;
          else update();
        });
      }
      this.clear = function () {
  ...
  function formatPolyLog(k, i) {
    ...
    if (typeof k == "number") {
      if (k == 0 && i != 0) ans = "";
      else if (k == 0 && i == 0) ans = "1";
      else if (k == 0.5) ans = "\\sqrt{n}";
      else if (k == 1) ans = "n";
      else k = k.toString();
    }
    if (ans !== null);
    else if (typeof k == "string") ans = "n^{" + k + "}";
